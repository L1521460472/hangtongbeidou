      ...
      <el-row>
        <el-col :span="24">
          <div class="header_nav">
            <h1>充电折扣配置</h1>
            <p>
              折扣配置说明：折扣范围0-99以内的2位数，例如：要打9.2折，输入92，打8折，输入80，不打折，则输入100即可。<span
                >当折扣为1时，APP上不显示优惠与折扣内容</span
              >
            </p>
...
export default {
  ...
  methods: {
    ...
    open() {
      ...
      this.$msgbox({
          ...
          h("p", { class: "message_p" }, [
            ...
            h(
              "i",
              {
                ...
                style: "color:rgba(255,255,255,.8);font-size: 12px;",
              },
              "新能源车折扣"
            ),
            h(
              ...
              {
                class: "message_right",
                style: "color: #FF4646;font-size: 12px;",
              },
              this.mes1
            ),
          ]),
        ...
        .then(() => {
          axios({
            method: "post",
            url: "/rest/chargeapi/carDiscountController/modify",
            headers: {
              Authorization: getCookie(1001),
            },
            data: {
              discount1: this.value1,
              discount2: this.value2,
              discount3: this.value3,
            },
          })
            .then((result) => {
              //   console.log(result.data)
              this.value1 = this.oldValue1 = result.data.data.discount1;
              this.value2 = this.oldValue2 = result.data.data.discount2;
              this.value3 = this.oldValue3 = result.data.data.discount3;
              this.$message({
                type: "success",
                message: "修改成功!",
                center: true,
    ...
    getData() {
      axios({
        method: "post",
        url: "/rest/chargeapi/carDiscountController/list",
        headers: {
          Authorization: getCookie(1001),
        },
      })
